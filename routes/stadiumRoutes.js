...
router.put("/stadium/:id", async (req, res) => {
  ...
  if (stadiumValidation.error)
    return res.status(400).send(stadiumValidation.error.details[0].message);

  await Model.stadium.findByIdAndUpdate(
    req.params.id,
    { ...req.body },
    (err, doc) => {
